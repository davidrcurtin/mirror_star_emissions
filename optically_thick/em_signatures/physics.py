...
cloudy_path = _THIS_DIR / "data" / "Cloudy ISM Abundances.txt"

cloudy_data = np.genfromtxt(
    cloudy_path,
    comments="#",
    skip_header=3,
    skip_footer=24,
...
T, rho, kap = np.loadtxt('data/freedman_opacities_2014_[M_H]=0.txt', skiprows=38, unpack=True, usecols = (0,2,3))
kappa_interpolator_freedman = CloughTocher2DInterpolator(list(zip(np.log10(rho*1000), np.log10(T))), np.log10(kap/10))
del T, rho, kap
